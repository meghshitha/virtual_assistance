...
def searchGoogle(query, num_sentences=5, max_width=100):
    url = f"https://www.google.com/search?q={query}"
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")
    search_result = soup.find("div", class_="BNeawe s3v9rd AP7Wnd").get_text()
    sentences = search_result.split(".")
    result = ". ".join(sentences[:num_sentences])
    wrapped_result = textwrap.fill(result, width=max_width)
    
    wrapped_result_with_spacing = "\n\n".join(wrapped_result.splitlines())

    return wrapped_result_with_spacing
...
def open_application(application_name):
    ...
    application_paths = {
        ...
        "Gallery" :r"C:\Users\USER\Pictures",
        "Documents":r"C:\Users\USER\Documents",
        "Viedos" : r"C:\Users\USER\Videos",
        "Viedo" : r"C:\Users\USER\Videos"
...
#     data = response.json()
#     page_id = next(iter(data["query"]["pages"]))  # Get the first page_id
#     content = data["query"]["pages"][page_id]["extract"]
#     return content


def get_educational_content(topic, max_sentences=7,max_width=80):
    url = f"https://en.wikipedia.org/w/api.php"
    params = {
        "action": "query",
        "format": "json",
    ...
    response = requests.get(url, params=params)
    data = response.json()
    page_id = next(iter(data["query"]["pages"]), None)  # Get the first page_id or None
    if page_id is None:
        return "Sorry, no educational content found for this topic."

    content = data["query"]["pages"][page_id].get("extract", None)

    # Limit the content to a single paragraph or up to max_sentences
    # if content:
    #     paragraphs = content.split("\n\n")
    #     content = "\n\n".join(paragraphs[:1])  # Use [0] for the first paragraph
    #     sentences = content.split(". ")
    #     if len(sentences) > max_sentences:
    #         content = ". ".join(sentences[:max_sentences]) + "."
    wrapped_result = textwrap.fill(content, width=max_width)
    ...
    while True:
        ...
        if query:
            if 'shutdown' in query.lower() or 'exit' in query.lower() or 'stop' in query.lower():
                ...
                textcreate(f"Master the time is {strTime}")
                ...
                get_current_date()
                    ...
                    textcreate("screenshot Taken And Saved")
                    take_screenshot()

            elif 'countdown' in query.lower() or ' start countdown' in query.lower() or ' start timer' in query.lower():
                    ...
                    textcreate("I AM virtual  ASSISTANT created by Finall year students of Seshadripuram degree college Tumkuru")
                    speak("I AM virtual  ASSISTANT created by Finall year students of Seshadripuram degree college Tumkuru")
                ...
                webbrowser.open(url)
                    ...
                    textcreate('sorry...Master i\'m not so good at it.. if you want i can play some good music for you...')
                    speak('sorry... Master I\'m not so good at it.. if you want i can play some good music for you...')
            ...
            #     speak(info)
            ...
            elif 'quote' in query.lower() or 'inspirational message' in query.lower() or 'inspirational quote' in query.lower():
                quote = get_daily_quote()
                textcreate(quote)
                speak(quote)

            elif 'educational content' in query.lower() or 'note on' in query.lower() or 'tell me about' in query.lower() or 'more about' in query.lower():
                speak("Sure, what topic would you like to learn about?")
                topic = takeCommand()
                content = get_educational_content(topic, max_sentences=4)
                textcreate(content)
                speak(content)

            elif 'show in google' in query.lower() or 'open in google' in query.lower(): 
                ...
                try:
                    ...
                    wrapped_results = textwrap.fill(results, width=80)  # Adjust width as needed
                    textcreate(wrapped_results)
                    speak(wrapped_results)
